...
async def do_create_document(content_block: str, session_id: Optional[str] = None, owner: Optional[str] = None) -> Dict:
    ...
    try:
        ...
        ver = DocumentVersion(
            ...
            version_number=1,
            content=content,
            summary=f"Created by {_pkg._active_model or 'AI'}",
            source="ai",
        )
        db.add(doc)
        db.add(ver)
...
async def do_update_document(content: str, doc_id: Optional[str] = None, owner: Optional[str] = None) -> Dict:
    ...
    target_id = doc_id or _pkg._active_document_id
    ...
    try:
        ...
        ver = DocumentVersion(
            ...
            content=new_content,
            summary=f"Updated by {_pkg._active_model or 'AI'}",
            source="ai",
        )
...
async def do_edit_document(content: str, doc_id: Optional[str] = None, owner: Optional[str] = None) -> Dict:
    ...
    try:
        ...
        ver = DocumentVersion(
            ...
            summary=f"Edited by {_pkg._active_model or 'AI'} ({applied} edit(s))",
            source="ai",
        )
...
async def do_manage_documents(content: str, owner: Optional[str] = None) -> Dict:
    ...
    try:
        if action == "list":
            ...
            doc_id = args.get("document_id") or args.get("id") or args.get("uid") or _pkg._active_document_id
            doc = None
            if doc_id:
            ...
            doc.is_active = False
            db.commit()
            if _pkg._active_document_id == doc.id:
