...
# ── MCP Tools (FastMCP decorator API) ──
#
# Each tool below replaces one branch of the old `@server.call_tool`
# dispatcher. Helper functions (IMAP/SMTP/account/folder/search/bulk) live
# above and are unchanged. Per-tool error handling preserves the "Error: ..."
# wire format the agent loop's tests assert on.

# The user may have multiple IMAP accounts configured. Every tool that
# touches an account accepts an optional `account` param — match by name
# (e.g. "work"), email address, or account id. Leave it out to use the
# default account. Typed-alias so the long help text is single-sourced
# across all tools.
...
def list_email_accounts() -> str:
    """List the email accounts configured in Odysseus. Returns each account's name, email address, and whether it's the default. Use this first when the user asks about a specific inbox by name (e.g. 'check work')."""
    rows = _list_accounts_raw()
    if not rows:
        return "No email accounts configured. Legacy single-account mode active."
...
def list_emails(
    ...
    """List unread or unresponded emails from the inbox. Returns subject, sender, date, and cached AI summary for each. Use this to check what emails need attention. Pass `account` to scan a non-default mailbox."""
    ...
    if len(all_accounts) >= 2 and not account:
        results, errors = _list_emails_across_accounts(
            ...
            unresponded_only=unresponded_only, unread_only=unread_only,
        )
        account_names = [
        ...
        active_cfg = _load_config(account)
...
def download_attachment(
    ...
    """Download an email attachment to the local disk so you can read it. Returns the local file path which you can then read with read_file. Use this when you need to review a document, spreadsheet, or other file attached to an email."""
    result = _download_attachment(uid, index, folder, account=account)
    if "error" in result:
...
def send_email(
    ...
    """Send a new email via SMTP. Provide recipient(s), subject, and body. For replying to an existing thread, use reply_to_email instead. Pass `account` to send from a non-default mailbox."""
    result = _send_email(
    ...
    return f"Sent email to {result['to']} with subject '{result['subject']}'{acct_note}."
...
def archive_email(
    ...
    ok = _archive_email(uid, folder, account=account)
    return f"{'Archived' if ok else 'Failed to archive'} UID {uid}"
...
def delete_email(
    ...
    ok = _delete_email(uid, folder, permanent=permanent, account=account)
    return f"{'Deleted' if ok else 'Failed to delete'} UID {uid}"
...
def mark_email_read(
    ...
    ok = _set_flag(uid, folder, "\\Seen", add=read, account=account)
    state = "read" if read else "unread"
    return f"{'Marked' if ok else 'Failed to mark'} UID {uid} as {state}"


@mcp.tool
def bulk_email(
    action: Literal["mark_read", "mark_unread", "archive", "delete", "junk"],
    ...
    """Perform one action on MANY emails at once — the efficient way to 'mark all as read', 'archive these', 'delete all spam', etc. Select messages either by an explicit `uids` list OR by `all_unread: true` (operates on every unread message in the folder). Far better than calling mark_email_read / archive_email once per message."""
    uids = uids or []
    if all_unread:
        uids = _search_uids(folder, "UNSEEN", account=account)
    if not uids:
        return "No messages selected (pass uids or all_unread=true)."
    requested_n = len(uids)
    changed_n = 0
    try:
        if action == "mark_read":
            changed_n = _bulk_set_flag(uids, folder, "\\Seen", add=True, account=account)
            verb = "marked read"
        elif action == "mark_unread":
            changed_n = _bulk_set_flag(uids, folder, "\\Seen", add=False, account=account)
            verb = "marked unread"
        elif action == "archive":
            cfg = _load_config(account)
            changed_n = _bulk_move(uids, folder, cfg["archive_folder"], account=account, role="archive")
            verb = "archived"
        elif action == "junk":
            cfg = _load_config(account)
            junk_folder = cfg.get("junk_folder") or "Junk"
            changed_n = _bulk_move(uids, folder, junk_folder, account=account, role="junk")
            verb = "moved to Junk"
        elif action == "delete":
            if permanent:
                changed_n = _bulk_set_flag(uids, folder, "\\Deleted", add=True, account=account)
                verb = "permanently deleted"
            else:
                cfg = _load_config(account)
                changed_n = _bulk_move(uids, folder, cfg["trash_folder"], account=account, role="trash")
                verb = "moved to Trash"
        else:
            return f"Unknown bulk action: {action!r}. Use mark_read/mark_unread/archive/delete/junk."
    except Exception as e:
        return f"Bulk {action} failed after partial work: {e}"
    if changed_n <= 0:
        return f"No matching UIDs found in {folder}; 0 of {requested_n} email(s) {verb}."
    suffix = "" if changed_n == requested_n else f" ({changed_n} of {requested_n} requested UIDs matched)"
    return f"Done — {changed_n} email(s) {verb}{suffix}."
...
def search_emails(
    ...
    """Search emails by free-text query (sender, subject, or body). Walks INBOX + Sent + Archive by default so older threads are findable, not just recent unread. Use this whenever the user names a person or topic that isn't in the most recent inbox slice — e.g. 'Sara Sotheby's', 'invoice from EY', 'last email about the property'. Returns matching emails with their UIDs so you can read_email or reply_to_email."""
    try:
        hits = _search_emails(query, folders=folders, max_results=max_results, account=account)
    except Exception as e:
...
def read_email(
    ...
    """Read the full content of a specific email. Provide either the UID (from list_emails) or a Message-ID. Returns the subject, sender, date, and full body text."""
    all_accounts = _list_accounts_raw()
    if len(all_accounts) >= 2 and not account:
        result = _read_email_across_accounts(
            uid=uid or None, message_id=message_id or None, folder=folder,
        )
    else:
        result = _read_email(
            uid=uid or None, message_id=message_id or None, folder=folder, account=account,
        )
    if "error" in result:
        return f"Error: {result['error']}"
